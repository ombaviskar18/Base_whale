...
interface LeaderboardPlayer {
  ...
  winRate: number;
  lastActive: string;
  badge: 'whale' | 'shark' | 'dolphin' | 'fish' | 'newbie';
  nftAvatar?: string;
}
...
const generateLeaderboardData = (): LeaderboardPlayer[] => {
  ...
  const usernames = [
    'VitalikB', 'Punk6529', 'Pranksy', 'WhaleShark', 'BeanieMagic',
    'CryptoKing', 'NFTCollector', 'DeFiMaster', 'BlockchainBeast', 'EthereumElite',
    'MetaWizard', 'TokenTiger', 'SmartContractSage', 'CryptoChampion', 'DigitalDolphin',
    'BlockchainBull', 'CryptoWhale', 'NFTNinja', 'DeFiDynamo', 'EthereumExpert',
    'TokenTrader', 'CryptoGuru', 'BlockchainBoss', 'DigitalDragon', 'MetaManager'
  ];

  const badges: LeaderboardPlayer['badge'][] = ['whale', 'shark', 'dolphin', 'fish', 'newbie'];
  ...
  return addresses.map((address, index) => {
    ...
    return {
      ...
      badge: index < 3 ? 'whale' : index < 8 ? 'shark' : index < 15 ? 'dolphin' : index < 22 ? 'fish' : 'newbie'
    };
  });
};
...
const getBadgeColor = (badge: LeaderboardPlayer['badge']) => {
  switch (badge) {
    case 'whale': return 'text-purple-400 bg-purple-500/20 border-purple-400/30';
    case 'shark': return 'text-blue-400 bg-blue-500/20 border-blue-400/30';
    case 'dolphin': return 'text-cyan-400 bg-cyan-500/20 border-cyan-400/30';
    case 'fish': return 'text-green-400 bg-green-500/20 border-green-400/30';
...
export const Leaderboard: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'whale' | 'shark' | 'dolphin'>('all');
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="text-center space-y-4">
        ...
        >
          ...
          <h1 className="text-4xl font-bold bg-gradient-to-r from-yellow-400 to-orange-400 bg-clip-text text-transparent">
            Whale Hunter Leaderboard
          </h1>
          <Trophy className="w-8 h-8 text-yellow-400" />
        </motion.div>
        <p className="text-gray-400 text-lg">
          Top crypto whale hunters competing for supremacy
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="flex space-x-2">
          {(['all', 'whale', 'shark', 'dolphin'] as const).map((filterType) => (
            <motion.button
              key={filterType}
              ...
              whileTap={{ scale: 0.95 }}
            >
              {filterType === 'all' ? 'All Players' : `${filterType}s`}
            </motion.button>
          ))}
        </div>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="text-sm text-gray-400">
                Level {player.level} • {player.winRate}% WR
              </div>
            </div>
          </motion.div>
        ))}
            ...
            <tbody>
              {filteredPlayers.map((player, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className={`inline-flex px-2 py-1 rounded-full text-xs border ${getBadgeColor(player.badge)}`}>
                      {player.badge.toUpperCase()}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="text-blue-400">{player.level}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
